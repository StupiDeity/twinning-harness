...
| Issue stuck `pipeline:halted` with a `verdict result=halt reason=scope-violation` comment and a `decision action=approve gate=scope` comment newer than the halt | Post-ENG-180 expected behaviour: a single `bash bin/pipeline.sh decide <ENG-N> --action approve --gate scope` removes `pipeline:halted` (via `_pipeline_clear_halt_label`); the next launchd tick's scope-approval replay enters `run-stage.sh::main`'s D-002 branch, calls `apply_transition` directly (bypassing `verdict_handler`'s protocol-violation path), and the issue advances `implementing → ui` (or `ui → reviewing`). Look for `dispatch model=... (stage=...)` followed by `stage <stage> complete for <ident> (scope-approval-replay transitioned <from> → <to>)` in `$PROJECT_STATE_DIR/<slug>/logs/local-*.log`, and `verdict=transitioned scope-approval-replay=1` in `events.jsonl`. **Pre-ENG-180 legacy recovery** (host not yet redeployed): manually clear via `bash bin/linear.sh remove-label <ENG-N> pipeline:halted`, then `bash -c 'source bin/verdict-handler.sh; apply_transition <ENG-N> implementing ui ""'` (or `ui reviewing ""`). |
...
| Halt at rc=48/49/50 with halt-reason `review-ledger-invalid` | ENG-190 review-stage ledger detective halt. Inspect the halt-comment body for the validator stdout (one line per defect: `review-ledger-malformed: row N: ...` = rc=48 JSON parse; `review-ledger-incomplete: row N: ...` = rc=49 field-or-critical-floor-or-severity-ladder-or-seed-header; `review-ledger-missing: ...` = rc=50 absent file). The halt comment names the offending row number AND, when parseable, the row's `finding_class_key`. Recovery: see `docs/runbooks/recovery.md` §12 — the ledger is NOT cleared on resume, so the detective re-halts on the same row until you fix or remove it. Post-ENG-191 the validator additionally enforces `blocks_ship` / `ship_classification_rationale` / `decision_factors` on every this-dispatch row whose `adjudicated_severity ∈ {major, critical}` — diagnostics include `blocks_ship-missing-on-blocking-severity`, `critical-floor-blocks-ship-violation`, `decision_factors must be object`, `decision_factors missing required keys: …`. Prior-dispatch rows are exempt by schema-grace (`dispatch_id != --dispatch-id`). |
| Issue at `stage:qa` with verdict comment `reason=ship-with-deferred-majors` and a fresh `deferred-majors/<ENG-N>` Linear comment | ENG-191 selective exit — no recovery needed; the reviewing agent shipped with N deferred major findings (all adjudged `blocks_ship=false` by the five-question rubric) and the orchestrator's post-dispatch hook posted the enumeration under sig `deferred-majors/<ENG-N>`. Deferred-majors audit recipe + power-user override in `docs/runbooks/recovery.md` §13. ENG-194: rows whose `defer_reason="out-of-plan-scope"` name a path NOT in the plan's File Structure — the reviewer auto-deferred rather than looping back; the deferred-majors comment renders both kinds as a flat list (use `jq` on the ledger to distinguish). Critical findings whose fix-target is out-of-plan still loop back (critical-floor invariant) and surface as a `scope-violation` halt — recovery follows the scope-violation rows in this table, with the ENG-180 caveat that `decide --action approve --gate scope` is currently broken. See `docs/runbooks/recovery.md` §15. |
| Issue at `stage:implementing` post-loopback with a fresh `dimensional-threshold/<stage>/<ident>` Linear comment AND a `verdict result=fail target=implementing reason=dimensional-threshold-not-met` orchestrator comment | ENG-118 dimensional-threshold gate coerced the agent's `verdict pass` / `verdict approve` because one or more payload-emitted dimensions scored below the operator's configured floor in `.review.thresholds.*` / `.qa.thresholds.*`. The sib comment enumerates the failed dimensions (`name`, `score`, `threshold`, `miss-reason ∈ {below-threshold, missing-in-payload}`). Recovery: see `docs/runbooks/recovery.md` §14 — raise/lower the threshold or remove the entry, then `bash bin/pipeline.sh decide <ENG-N> --action continue`. The agent's self-reported `threshold_met` / `thresholds_met[]` fields are forensic-only; the orchestrator computes its own gate decision from `score` and the configured floor. |
| Halt at rc=41 with halt-reason `qa-payload-invalid` and a defect string starting with `qa-payload-missing` (or rc=39/42/50 with defect `qa-payload-malformed`) | ENG-203 orchestrator-merge helper tripped: the qa agent did not write `verdict-qa.body.json`, or the body was malformed / a symlink / oversize / unwritable canonical. The §6 prompt now instructs the agent to write a content-only body sidecar at `{qa_payload_body_path}`; the orchestrator merges the schema envelope (`qa_payload_schema_version`, `issue_id`, `dispatch_id`) before the schema validator runs. See `docs/runbooks/recovery.md` §15 for the per-rc diagnosis table, the deploy-cutover edge case (one issue per project halts on first post-cutover qa dispatch), and the `envelope-overwrite` forensic signal. **Recovery:** `bash bin/pipeline.sh decide <ENG-N> --action continue` — the qa-stage clear pre-cleans all four qa files, so hand-edits to the body file before resume are erased; repair the canonical instead and emit the verdict manually if the body is unrecoverable. |
| Halt at rc=38 with halt-reason `review-payload-invalid` and defect `review-payload-missing` (or rc=36 with defect `review-payload-malformed`) | ENG-205 orchestrator-merge helper tripped: the review agent did not write `verdict-review.body.json`, or the body was malformed / a symlink / oversize / unwritable canonical. The §5 prompt now instructs the agent to write a content-only body sidecar at `{review_payload_body_path}`; the orchestrator merges the schema envelope (`review_schema_version`, `issue_id`, `dispatch_id`) before the schema validator runs. rc=38 is remapped from helper rc=41 (body missing); rc=36 is remapped from helper rc=39/42/50 (malformed/symlink/write-failure). See `docs/runbooks/recovery.md` §16 for the per-rc diagnosis table, the deploy-cutover edge case (one issue per project halts on first post-cutover reviewing dispatch), and the `envelope-overwrite` forensic signal. **Recovery:** `bash bin/pipeline.sh decide <ENG-N> --action continue` — the reviewing-stage clear pre-cleans both review files, so hand-edits before resume are erased; repair the canonical instead and emit the verdict manually if the body is unrecoverable. |
