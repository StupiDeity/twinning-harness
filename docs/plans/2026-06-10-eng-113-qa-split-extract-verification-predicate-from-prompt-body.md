...
The plan above was authored against the dff490c base; rebase + three review iterations introduced drift each intentionally taken but worth recording as a faithful ship-state mirror. Where this section disagrees with §§1–7 above, the body of §§1–7 is authoritative — this section is a changelog, not contract.

- **Exit codes 36/37/38 → 39/40/41 → 42/43/44** (drift documented in commits 8df8fef + iter-3): codes 36/37/38 were taken by ENG-119 (review-payload-{malformed,incomplete,missing}), shifting ENG-113 to 39/40/41 in iter-2. Between iter-2 and iter-3, ENG-117 merged on origin/main claiming 39/40/41 for qa-payload-{malformed,incomplete,missing}. The post-iter-3 rebase observed the collision; ENG-113 (qa-predicate) now sits at 42/43/44. `failure_outcome_for_exit` in `bin/common.sh` carries both qa-payload (39/40/41) and qa-predicate (42/43/44) mappings side-by-side — they are parallel concepts per brainstorm OQ-3 (predicate is the scriptable JSON the agent emits at dispatch start; payload is the dimensional verdict the agent emits at exit).
...
- **`_url_host_class_denied` userinfo-strip reorder** (iter-8 critical): pre-fix the URL parser stripped userinfo (`${rest##*@}`) BEFORE isolating the authority component. Bash `##*@` is greedy: applied to the whole URL it consumed through every `@` in path/query/fragment, leaving the post-`@` tail as a counterfeit host. RFC 3986 puts `@` only inside the authority component, and curl parses the same way (authority ends at the first `/`), so `http://127.0.0.1/@public.com/` had its validator-side host parsed as `public.com` while curl connected to `127.0.0.1`. Same parser-divergence class as the iter-3 case-insensitive-scheme and iter-5 unbracketed-IPv6 bypasses. Fix: strip path/query/fragment FIRST, then strip userinfo within the isolated authority. V-26L..V-26P pin the five attack surfaces (loopback, IMDS, RFC1918, IPv6 loopback, fragment).
- **Per-kind `_warn_unknown` calls dropped from `_validate_pass_criterion`** (iter-8 M1): the pre-lift `bin/plan-schema.sh:216-265` emitted per-kind unknown-field warnings on typo'd keys (`pass_criteria[smoke]` level etc.). Plan §5 Task 2 claimed "behavior-preserving" for the lift into `common.sh`; the lifted helper has no `_warn_unknown` calls. Outcome chose: document the drift here (§9's "changelog not contract" framing) rather than re-add the calls. Per-feature and top-level `_warn_unknown` calls are unaffected — they live in `plan-schema.sh::cmd_validate` and still emit. Impact is diagnostic-only (stderr warning, no rc change); typo'd criterion-level keys are now silently accepted. Restoration is a one-iteration follow-up if a future plan-author hits a silent-accept incident.
- **`curl --max-filesize 1048576`** (iter-8 M8): added to the http_get curl invocation. `--max-time 10` bounds wall-clock but a hundred-MB body within 10s LAN window can exhaust disk before the EXIT trap cleans `body_tmp`. Same DoS-byte-cap intent as the predicate-file 64 KiB cap (D-011). `expect_body_match` grep still works on a truncated body — 1 MiB is well above any response-body signal the predicate would inspect. V-8 stub pins the flag's presence (any positive integer).
- **V-44b snap-bound diagnostic pin** (iter-8 M5): V-44 alone passes identically before and after the iter-6 M2 `head -c MAX+1 → snap_file` mechanism (both trip on `wc -c < ARG_FILE` and the post-fix `wc -c < snap_file`). V-44b distinguishes them by asserting the diagnostic content: with a 1 MiB ARG_FILE the post-fix code reports `size 65537 exceeds cap 65536` (the snap-bounded size), while a regression to `wc -c < ARG_FILE` would report `size 1048576`.
- **V-30g/V-30h IPv6 unspecified long-form + partial-collapse** (iter-8 M7): V-30f pins only the canonical short form `[::]`. V-30g (`[0:0:0:0:0:0:0:0]`) and V-30h (`[0::]`) cover the long-form and one partial-collapse arm of the unspecified axis. A regression that deleted the long-form arm from `_url_host_class_denied` would not trip V-30f.
- **V-8 stub `--proto` / `--proto-redir` / `--max-filesize` pins** (iter-8 M6/M8): the V-8 curl stub's argv walker now records `proto_seen`, `proto_redir_seen`, and `max_filesize_seen` and the V-8 assertion checks each. Without these pins a regression that drops any of the three load-bearing security flags ships clean through the suite.
