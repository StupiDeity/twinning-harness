...
**branch-base freshness:** `HEAD..origin/main` is NON-EMPTY at plan time (8 commits from the ENG-138 sibling work). The upstream commits touch `bin/guards.sh`, `bin/guards-test.sh` (new), `bin/guards-adversarial-test.sh` (new), `bin/run-stage.sh` (lines 1235-1242 only — `main`'s `guards.sh check` call now passes `$stage`), `CLAUDE.md` (one failure-mode-quick-ref row), plus a new ENG-138 brainstorm/plan pair. None of the changed `bin/run-stage.sh` lines overlap with this plan's insertion points (the new `_validate_qa_payload` definition slots in around line 1085, after ENG-122's `_post_plan_contract_halt`; the new caller slots in around line 1836, after ENG-122's plan-contract caller block). No overlap with this plan's File Structure for `bin/common.sh`, `bin/pipeline-events.json`, `AGENT_PROMPTS.md`, or the new files. Clean drift. **Task 0** rebases onto `origin/main` before any other implement work and re-verifies the `path:line` excerpts below. Pin: `origin/main = 444e752` at plan time.
...
- [ ] Run `git rebase origin/main`. Expected: clean fast-forward / 3-way merge. The upstream commits touch `bin/guards.sh`, `bin/guards-test.sh` (new), `bin/guards-adversarial-test.sh` (new), `bin/run-stage.sh` (8 lines at the `guards.sh check` call in `main` — far above this plan's insertion points), `CLAUDE.md` (one Failure-Mode quick-ref row), and a new ENG-138 brainstorm/plan pair. No overlap with this plan's File Structure.
- [ ] Re-verify each `path:line` excerpt in Assumption Inventory by `Grep`ing for the named content anchors (`# ENG-122: plan-contract validator.`, `35) printf 'plan-contract-missing' ;;`, `"plan-contract-invalid"`, `rm -f "$d/wait-${stage}.json"`, `# Push branch BEFORE posting the completion comment`, `- No edits to qa-patterns.md`, `Quality gates (must all be true to advance):`, `## 6. QA Agent`). Each anchor MUST resolve to exactly one match post-rebase. If any anchor moves to a different line, the content-anchor approach below survives — line-number hints are informational only.
- [ ] If a conflict arises (unexpected — none of the changed upstream files overlap with this plan's insertion points beyond `bin/run-stage.sh`'s far-upstream `guards.sh check` call), STOP and run `bash bin/pipeline.sh event ENG-117 verdict halt --reason agent-blocked` with a one-line Linear comment naming the conflict file.
