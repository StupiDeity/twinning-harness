...
| §5 fenced block is corrupted (new column-0 ``` fence introduced inside the body) | Edit that adds a column-0 ``` fence in §5 between the heading and the closing fence | `bin/render-prompt.sh::extract_block` `die`s on fence-count != 2 the first time §5 is rendered (next review dispatch — runtime, not suite) | runtime (no suite-time pin for §5) | n/a — the §2-specific backstop at `bin/agent-prompts-content-test.sh:147-153` is the only suite-time fence-count assert; generalising to §§3-8 is separate work |
