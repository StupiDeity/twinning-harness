...
| Mid-dispatch crash between allocator and clear-at-start | Inject crash via `kill -TERM` between alloc and `_clear_current_stage_slots` (manual) | `dispatch_history.jsonl` has start row without end row; next tick's allocator increments past it; clear-at-start fires unconditionally on next dispatch | integration | `bin/run-stage-test.sh::Case-87-D` (idempotent clear) — partial coverage; full crash-recovery test deferred to manual operator drill |
...
| Reader's dispatch_id fallback (legacy issue, no markers) | `MOCK_COMMENTS_JSON` with no `meta: dispatch id=` markers | `find_fresh_verdict` falls back to timestamp window; returns latest verdict | unit | `bin/linear-test.sh::Case-87-L5` + `bin/verdict-handler-test.sh::Case-87-V2` |
...
| Reader's dispatch_id match (legitimate resume) | Transition's id matches current | `resume_in_progress_transition` returns 0; `apply_transition` invoked | unit | `bin/verdict-handler-test.sh::Case-87-V4` |
| ENG-41 §1.2-style 2-day-old transition selected as in-progress | Stale comment with no marker; current_stage=brainstorming, comment.from=planning | Existing labels-cross-check fires (legacy fallback, D-005) → returns 1 | unit | `bin/verdict-handler-test.sh::Case-87-V5` (legacy fallback preserves ENG-41 §4.2 guard) |
...
| `find_fresh_verdict` picks unstamped decoy by createdAt when markers exist | Three comments: two stamped (older), one unstamped (newest) | Stamped pass marker wins (id-match path); unstamped decoy filtered out | unit | `bin/linear-test.sh::Case-87-L6` |
