...
| Agent posts via `mcp__plugin_linear_*` (bypassing linear.sh) | Fixture transcript with assistant `tool_use` Bash command starting `mcp__plugin_linear_linear__save_issue` | Envelope validator halts with rc=29; classify_failure → `dispatch-envelope-violation`; halt comment posted | unit | `bin/run-stage-test.sh::Case-87-H` |
| Agent direct-curls Linear GraphQL | Fixture transcript with `curl https://api.linear.app/graphql` | Envelope validator halts with rc=29; same classify path | unit | `bin/run-stage-test.sh::Case-87-I` |
