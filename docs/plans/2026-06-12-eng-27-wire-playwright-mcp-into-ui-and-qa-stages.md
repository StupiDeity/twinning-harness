...
- `docs/install.md` — document the new `playwright-install` phase and renumber the subsequent `launchd` phase heading to match `bin/setup.sh`'s `ALL_PHASES` order. (Scope addendum 2026-06-12: the original plan changed `bin/setup.sh` but omitted its operator-facing doc; the reviewing-stage cold pass correctly flagged the resulting drift as a maintainability major, so `docs/install.md` belongs in this File Structure.)
...
| Agent forgets `{artifacts_dir}/` prefix when calling `mcp__playwright__browser_take_screenshot` | Screenshot lands in agent CWD = worktree | Post-dispatch `partition_dirty_paths` routes `.png` at worktree root to the out-of-scope stream on ui/qa → `bin/run-local.sh` self-leak gate hard-fails it as a NEW untracked path (existing behavior, unchanged by this plan) | integration | `bin/run-local-sweep-test.sh` case 29 (`screenshot_root_leak_ui` / `screenshot_root_leak_qa`) — concrete `.png`-at-root assertions added by this plan |
...
- **Integration**: `bin/run-local-sweep-test.sh` case 29 (`screenshot_root_leak_ui` / `screenshot_root_leak_qa`) pins a concrete `.png`-at-worktree-root assertion for the agent-forgets-the-prefix failure mode — the stray screenshot routes to the out-of-scope stream on ui/qa, which `bin/run-local.sh` then self-leak-hard-fails as a new untracked path.
