...
- **Surface:** `harness inbox` / `harness status` (the single binary; reads SQLite; renders **local
...
- **Actions (CLI, deliver the `human_resume` signal):**
  - `harness resume <ticket>` — re-enter the parked step; reset per-loop + B2 counters; fresh B3 allowance.
  - `harness resume <ticket> --after-fix` — operator edited plan/code/config by hand; the daemon
    picks up the changed worktree/SoT state.
  - `harness abandon <ticket>` — terminal (`status='abandoned'`; projector → Canceled).
