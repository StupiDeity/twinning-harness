...
> **▶ RESUME HERE (2026-06-19):** substrate spec CLOSED (§10); **SQLite schema DONE** → `docs/redesign/schema.sql` (§12); **durable control-loop semantics FROZEN** → `docs/redesign/control-loop.md` (§9.4 #2 — daemon + event loop; a full **step catalog S1–S10** with per-step guards/inputs/outputs/tools walked + approved with the operator; validated-tool-interface for structured output (§3a); **review redesigned** (findings filed via tool calls, verdict derived from state, no deferral dictionary); generic checks-system (CL-CHECKS, polling); stale-branch handling (CL-STALE); the **Loopback Atlas** (§8); GOAL-INSTALL §10). **Next artifact = §9.4 #3** the one-time state-import mapping (issue-state.json + labels + marker history → rows), then #4 the one-way projector (drain `projection_outbox`). Only open spec item is **E2** (`decision_class`/`action_surface` vocabularies), post-cutover (UGL). Read control-loop.md → §12 → §10 → §9.
>
...
3. **One-time state import**: read current Linear labels + per-issue JSON into SQLite for every in-flight ticket (one-shot, hand-run, verified).
