...
Six instances, one class. The pattern, every time:

1. The orchestrator OR the agent persisted data during dispatch N.
2. Dispatch N+1 read that data.
3. Either (a) the data was supposed to have been overwritten in N+1
   but wasn't (stage-summary file, branch-name interpolation), or
   (b) the data was supposed to be filtered out as "from N, not N+1"
   but wasn't (Linear comment freshness window, halt label,
   transition marker).

### 1.2 Why prior fixes didn't generalize

Each prior fix targeted one medium with that medium's natural
primitive:

- ENG-77 D-001: prompt-rule mandate at agent layer → fixes ONE
  stage's stage-summary file behavior.
- ENG-41 §4.1: lane-fence at `linear.sh` chokepoint → fixes label-
  write authorship.
- ENG-79: canonical resolver call in `render-prompt.sh` → fixes ONE
  prompt token's source-of-truth.
- ENG-67: invariant `die` on missing worktree → fixes worktree-path
  freshness.

The fixes are correct in isolation. None of them gives a future
operator a way to ANSWER the question "which dispatch wrote this
artifact?" at read time — and that question is precisely the one
every instance of the class collapses to. Without a per-dispatch
identifier, freshness has to be reconstructed from secondary signals
(timestamps, mtime, label state, comment ordering); secondary
signals are exactly what each incident exploited.

The system already documents — verbatim, on `bin/run-stage.sh:497-499`
— the rationale that should generalize but doesn't:
...
That comment is in `_handle_wait` (verified at `bin/run-stage.sh:489-545`),
applied only to the wait-exit path. Build (§7) is the only stage that
exercises wait-exit, which is why ENG-77 §6 named build as
"structurally immune." The reasoning ("don't post stale content from
a prior dispatch") is the design we want; the scope is wrong.
...
5. **G-5 (envelope-validation).** The orchestrator's post-dispatch
   ...
   (covered by existing rc=25 path at `bin/run-stage.sh:1068-1080`),
   (b) every Linear comment posted under sigs scoped to (issue,
   current_stage) during this dispatch carries the current
   `dispatch_id` marker, (c) no Linear write happened outside
...
- **A whole-issue lock spanning poll → dispatch → verdict_handler.**
  Existing `.claude-mutex.lock/` is sufficient; the contract relies
  on serialized dispatch (already true). Whole-issue lock stays
  deferred per ENG-41 §12.
- **Cycle-id stamping on log files / forensic-only artifacts.** Logs
  may gain a `[dispatch=ENG-N-d014]` prefix for searchability; no
  validator runs on log content (open question §9.4).
...
- **Replacing ENG-77 D-001's already-shipped §5 prompt rule.** The
  rule remains; this design GENERALIZES it via clear-on-start
...
Each medium gets the cheapest primitive that achieves G-2 through
G-5 for that medium's persistence semantics. The primitives are NOT
...
|---|---|---|---|---|
| Per-issue local files | mutable, local FS | **G-2: clear-at-dispatch-start** for current-stage files (extend `bin/run-stage.sh:497-499`'s wait-exit pattern). After clear, file existence at envelope time = THIS-dispatch authorship. | Existing rc=25 (`agent-contract-missing`) at `bin/run-stage.sh:1068-1080`; covers "agent didn't write at all" loud-fail. | +20 |
| Linear comments | append-only, remote | **G-3: auto-injection** in `bin/linear.sh::add_comment` (line 470) and `add_or_update_comment` (line 541). Every comment body carries `<!-- meta: dispatch id=… stage=… -->`. | Cross-check via `linear.sh list-comments` filtered to current dispatch's writes; assert each has the marker. Transcript scan asserts no Linear write outside `linear.sh`. | +25 |
...
| Prompt tokens | re-rendered each dispatch | **G-4: canonical resolver registry** in `bin/render-prompt.sh`. `{token}` → resolver function map; resolver is the source of truth. ENG-79's `branch_name`-via-`bin/branch-name.sh` becomes the general case. | Render-time validator: any `{token}` in the source not in `PROMPT_RESOLVERS` → `die`. Content test in `bin/agent-prompts-content-test.sh`: assert every `{…}` token in `AGENT_PROMPTS.md` appears in the registry. | +50 |

### 4.3 How `dispatch_id` strengthens ENG-41's planned §4.2 cross-check
...
`dispatch_id` is monotonic per issue; a transition marker from a
prior cycle cannot have an id ≥ current. The dispatch_id check is
strictly stronger than the timestamp window AND strictly stronger
than the labels-cross-check, because (a) it doesn't rely on clock
skew or timestamp-comment ordering, and (b) it doesn't rely on the
labels having been kept consistent.
...
auto-injection at `linear.sh` is stage-agnostic; the resolver
registry is stage-agnostic.
...
| `bin/dispatch.sh` | Inherit `PIPELINE_DISPATCH_ID` and `PIPELINE_STAGE` into the `env` block at `bin/dispatch.sh:437`. | +2 |
...
| `bin/common.sh::failure_outcome_for_exit` | Add new exit code for `envelope-violation` (next free, e.g. 29) so retrospective §1 filter recognizes it. | +2 |

### 5.2 Per-medium mechanisms

| File | Change | LOC |
|---|---|---|
| `bin/run-stage.sh` (clear-on-start) | New `_clear_current_stage_slots <issue> <stage>`; called at dispatch start. Generalizes the wait-exit pattern from `bin/run-stage.sh:497-499` to all stages. | +20 |
...
| `bin/verdict-handler.sh` (reader filter) | `find_fresh_verdict` (`bin/verdict-handler.sh:84`): filter to comments whose dispatch_id marker matches current. Fallback to timestamp window when no dispatch_id present anywhere on the issue (legacy). `resume_in_progress_transition` (`bin/verdict-handler.sh:330`): dispatch_id mismatch → return 1 (primary); ENG-41's labels-cross-check stays as secondary. | +50 / -25 |
| `bin/render-prompt.sh` (resolver registry) | Top-of-file `PROMPT_RESOLVERS` table: `{token} → resolver function`. Resolver functions are thin wrappers (`_resolve_branch_name` → `bash bin/branch-name.sh`; `_resolve_dispatch_id` → `echo "$PIPELINE_DISPATCH_ID"`; etc.). Render loop (currently the python block at `bin/render-prompt.sh:230-252`) iterates over tokens encountered in the source, dies on unknown. | +60 / -30 |

### 5.3 The detective backstop

| File | Change | LOC |
|---|---|---|
| `bin/run-stage.sh` (envelope validator) | New `_validate_dispatch_envelope <issue> <stage>`: (a) stage-summary file exists post-dispatch (existing rc=25 path at `bin/run-stage.sh:1068-1080`); (b) Linear comments posted under sigs `(completion\|halt\|scope-approval\|retry-pending)/<stage>/<issue>` during this dispatch all carry the current dispatch_id marker; (c) transcript scan via the existing `assert_no_tool_invocation` pattern (already used at `bin/dispatch.sh` for the implement-stage `gh pr create` ban) — extended to assert no `mcp__plugin_linear` invocation, no `curl ... linear.app`, no direct Linear API calls outside `bin/linear.sh`. Halt with `verdict halt --reason dispatch-envelope-violation` on any failure. Skip on wait-exit and scope-approval-replay. | +60 |

### 5.4 Prompt-side surface

| File | Change | LOC |
|---|---|---|
| `AGENT_PROMPTS.md` (preamble, near line 91) | New section: "Dispatch identifier and freshness contract." Documents `{dispatch_id}` interpolation, the auto-injection rule (agents MUST NOT manually emit `dispatch id=` markers), and the no-manual-Linear-API rule. ~30 lines. | +30 |
| `AGENT_PROMPTS.md` Stage summary contract (lines 164–209) | Generalize ENG-77 D-001's "MANDATORY — overwrite on every dispatch" rule (currently §5-only at `AGENT_PROMPTS.md:1027-1037`) to apply uniformly to all §§1–7. Per ENG-77 D-003, the structural change here un-defers the §3/§4 generalization (clear-at-start removes the substrate that made narrowing prudent). | +15 / -5 |
| `bin/agent-prompts-content-test.sh` | Generalize ENG-77 D-002's three asserts (currently at `bin/agent-prompts-content-test.sh:197-237`, §5 only) to §§1–7 (each stage's stage-summary bullet must mandate overwrite-every-dispatch). Add new asserts: §preamble cites the dispatch_id contract; §preamble names the auto-injection rule; every `{…}` token in the file appears in `PROMPT_RESOLVERS`. | +60 |
...
| `bin/linear-test.sh` | Marker injection: `add-comment` body unconditionally carries `<!-- meta: dispatch id=… stage=… -->` when env vars set. With env unset (operator manual call), no injection (preserves operator-side behavior). `find_fresh_verdict` filters by id; legacy-comment fallback returns timestamp-window result. | +140 |
...
T0  Tick: poll selects (ENG-71, ui).
    ...
    │    {branch_name}     → feat/eng-71-…  (PROMPT_RESOLVERS[branch_name] → bin/branch-name.sh)
...
The post-fix behavior depends on EITHER the dispatch_id filter OR
the labels-cross-check; the design ships both because the
labels-cross-check covers the legacy-comment fallback window.
...
counts are easily reconstructed from `dispatch_history.jsonl`
regardless of id format.

Stage information is preserved as a *field* on every artifact stamp
(`<!-- meta: dispatch id=ENG-71-d014 stage=reviewing -->`), so
forensic readability is not lost — just not encoded in the id
itself.

### D-002: dispatch_id allocator lives in `bin/common.sh`

Chosen: new `allocate_dispatch_id <issue>` in `common.sh` exported
alongside `issue_dir` (verified at `bin/common.sh:69-73`),
`compute_pipeline_content_hash`, etc. `run-stage.sh` calls it;
`dispatch.sh` calls `current_dispatch_id` (read-only) when re-rendering
on retry.
...
Chosen: `bin/linear.sh::add_comment` (line 470) and
`add_or_update_comment` (line 541) auto-inject the `dispatch_id`
marker when `$PIPELINE_DISPATCH_ID` is set.

Rejected: orchestrator post-processes every comment after the
dispatch returns. Bookkeeping-heavy (must track which sigs were
touched during the dispatch); race-prone (if the dispatch crashes,
post-process never runs and comments are unstamped). Chokepoint
injection inherits via env naturally and crashes the dispatch loud.

Rejected: agent emits the marker manually per prompt rule. Same
trust problem as ENG-41 §4.1 — agent could lie, skip, or use a
stale id. The chokepoint is the single trustworthy authoring site.
...
this dispatch carries the auto-injection) are belt-and-suspenders
for the chokepoint — if the chokepoint is sound, every comment HAS
the marker; if it doesn't, that's a chokepoint bug.
...
Chosen: `find_fresh_verdict` and `resume_in_progress_transition`
fall back to timestamp-window when no `dispatch_id` markers exist on
the issue. Fallback expires once any `dispatch_id` marker appears on
the issue (which happens the first time the orchestrator dispatches
the issue post-cutover).
...
labels-cross-check the trust-model brainstorm specified. ENG-41
ships the cross-check as primary; this design demotes it to fallback
once the dispatch_id primary is in place.
...
dispatch" (verified at `AGENT_PROMPTS.md:1027-1037`) generalizes to
every stage's stage-summary-file Output bullet. ENG-77 D-002's three
asserts (verified at `bin/agent-prompts-content-test.sh:197-237`)
generalize from §5 to all stages.
...
where the implement agent emits fresh tdd-evidence but doesn't
rewrite stage-summary-implementing.md." This design's clear-at-start
removes the substrate that made the narrowing prudent — there is no
longer a "residual file" the agent could read-then-skip-write,
because the file is cleared by the orchestrator before the agent
sees it.
...
where `issue_dir` (verified at `bin/common.sh:69-73`) already
requires non-empty input; `$ident` shape validation belongs at
`issue_dir` itself (separate hardening, out of scope).

### D-008: ENG-77 D-005 (typed findings channel) stays deferred

Chosen: this design does NOT introduce typed/schema'd findings.
ENG-77 D-005's un-deferral conditions still apply unchanged:

- Two distinct review-implement loop incidents *despite* this
  contract.
- Operator demand for structured findings.
- A typed JSON schema with byte-cap, validation, and explicit
  nested-marker rejection.

The contract this design ships is freshness-only; content shape
stays prose. If ENG-77 D-005's un-deferral fires, the typed channel
layers ON TOP of dispatch_id (each typed-findings comment still gets
the auto-injection), not in place of it.
...
1. **Phase 1 (glue).** `bin/common.sh` allocator; `bin/dispatch.sh`
   ...
   `pipeline-events.json` vocabulary expansion. No behavior change
   yet (allocator allocates but nothing reads).
2. **Phase 2 (clear-at-start).** Generalize wait-exit pattern.
   Existing rc=25 (`agent-contract-missing`) catches the loud-fail
   case (agent didn't write).
3. **Phase 3 (linear.sh injection + reader filter).** Auto-injection
   universal; readers filter by id with timestamp-window fallback
   for legacy.
4. **Phase 4 (resolver registry + envelope validator + content tests).**
   Generalize ENG-79's pattern; ship the detective backstop;
   un-defer ENG-77 D-003.
...
could still write to Linear via direct API and bypass the
auto-injection). ENG-41's brainstorm is at
...
Chosen: env var, set by `run-stage.sh` and `dispatch.sh`, inherited
by subshells naturally. Mirrors ENG-41's `PIPELINE_WRITER` pattern
(verified at `bin/dispatch.sh:437`) — same rationale.

Rejected: `--dispatch-id` flag on every `bin/linear.sh` call. 30+
call sites need editing; env var inherits transparently.

Rejected: `config.json` field. dispatch_id is per-dispatch, not
per-project; config is the wrong granularity.

## 8. Failure modes

| Failure | What happens | Detection |
|---|---|---|
| Agent skips Write of stage-summary file | File doesn't exist post-dispatch (cleared at start, never recreated) → existing rc=25 (`agent-contract-missing`) path fires → halt. | Existing rc=25 in `bin/run-stage.sh:1068-1080`. |
...
| `issue-state.json` corruption mid-allocate | Allocator dies (jq parse fails) → `run-stage.sh::main` dies → tick logs FATAL → next tick's allocator sees corrupt state too → human recovery. | FATAL log entry; halt-sprawl monitor. |
...
| Mid-dispatch crash between allocator and clear-at-start | `dispatch_history.jsonl` has a record without `exit_code, exit_at`. Next tick's allocator increments past it (counter is durable). The previous (orphaned) attempt's stage-summary file may or may not have been cleared — but next tick's clear-at-start fires unconditionally, so it gets cleared. | dispatch_history.jsonl gap; benign. |
...
1. **Should `dispatch_id` be visible in the Linear status column or
   ...
   **Working answer:** no for this design — Linear custom-field
   write is a new lane (orchestrator-only) and adds a sync surface.
   Defer; `bin/status.sh` displays it from `dispatch_history.jsonl`
   instead.

2. **Should the dispatch_id stamp on Linear comments include
   `pipeline_content_hash` for cache-key forensics?** Adding it
   makes "compare two dispatches' inputs" queryable from Linear
   alone. **Working answer:** no — the hash is large and noisy in
   comment bodies; `dispatch_history.jsonl` already stores it
   per-dispatch.

3. **What if the agent writes the stage-summary file AND emits a
   verdict-marker, but the verdict-marker's dispatch_id (auto-injected
   by linear.sh) doesn't match the file's "stamp" (which doesn't
...
5. **Is render-time validation (PROMPT_RESOLVERS unknown-token die)
   the right enforcement mechanism, or should
   `bin/agent-prompts-content-test.sh` carry the assertion?** Both.
   Test pins catch drift at commit-time; render-time die catches
   anything that escapes (e.g., a token added to AGENT_PROMPTS.md
   by a non-harness contributor without running tests). Cheap to
   ship both.

6. **Does ENG-78's halt-on-retry-immediately bug need to ship before
   this design or after?** Independent; ENG-78's fix is a logic bug
   in `classify_failure`, not a hand-off contract issue. Either
   order works; ship whichever is in flight first.
...
7. **GitHub-side artifacts (review's per-finding `gh pr review
   ...
   GitHub-side artifacts and no Linear-side comment trips rc=25 (no
   stage-summary file) OR fails the envelope validator's "verdict
   marker present this dispatch" check.

## 10. Assumption inventory

| ID | Assumption | Verification |
|---|---|---|
| A-001 | `bin/run-stage.sh:497-499` clears stage-summary on wait-exit, with the rationale comment "don't post stale content from a prior dispatch." | **Verified** by direct read 2026-05-09 (`sed -n '480,510p' bin/run-stage.sh` showed the rm and comment in `_handle_wait`). |
| A-002 | `bin/run-stage.sh:1068-1080` is the agent-contract validator carrying the existing `summary_missing` rc=25 path. | **Verified** by direct read 2026-05-09 (`grep -n` and `sed`); the line numbers in the prior draft (977-989) referred to a different path (severe scope violation rc=21) — corrected here. |
| A-003 | `bin/render-prompt.sh:224` already calls `bin/branch-name.sh` for `branch_name` (ENG-79). | **Verified** by direct read 2026-05-09; the comment block at `bin/render-prompt.sh:212-223` documents the ENG-79 fix. |
| A-004 | `bin/linear.sh::add_comment` lives at line 470; `add_or_update_comment` at line 541. | **Verified** by `grep -n '^add_comment\\|^add_or_update_comment' bin/linear.sh`. |
| A-005 | `issue_dir` lives at `bin/common.sh:69-73` and validates non-empty input. | **Verified** by direct read 2026-05-09 — `[[ -n "$issue" ]] || die "issue_dir: missing issue id"`. |
| A-006 | `.claude-mutex.lock/` serializes `claude -p` invocations per host; per-issue locks serialize per-issue runs. | **Verified** per CLAUDE.md "Per-issue state directory" §; allocator atomicity layered on top. |
| A-007 | The agent's `--allowed-tools` list excludes `mcp__plugin_linear` (per ENG-41 A-004). | **UNVERIFIED** at brainstorm time — same caveat as ENG-41 §A-004. The transcript scan in `_validate_dispatch_envelope` is the backstop. |
| A-008 | `bin/agent-prompts-content-test.sh` runs in `.githooks/pre-commit` and exists at the cited path. | **Verified** — file is 946 lines; ENG-77 D-002 §5 asserts at lines 197-237. |
| A-009 | `bin/pipeline-events.json` is the closed registry; new `meta_kinds` and `halt_reasons` require entries there. | **Verified** by `jq '. \| keys'` — `meta_kinds`: `[dedup,metric,evidence,reapplied,forensic]`; `halt_reasons`: `[agent-blocked, agent-failure, smoke-failed, iteration-exhausted, scope-violation, protocol-violation, dispatch-timeout, pr-opened-too-early]`. New `dispatch` kind and `dispatch-envelope-violation` reason fit cleanly. |
| A-010 | Adding an env var (`PIPELINE_DISPATCH_ID`) to `bin/dispatch.sh:437`'s env block is sufficient for subshell inheritance into `bin/linear.sh` calls the agent makes. | **Verified** — current line 437: `local cmd=(env PIPELINE_WRITER=agent gtimeout … claude -p …)`. Adding `PIPELINE_DISPATCH_ID=$PIPELINE_DISPATCH_ID PIPELINE_STAGE=$stage` follows the same shape. |
| A-011 | `dispatch_history.jsonl` is a new file; no existing tooling depends on it. | **By construction** — new artifact, no callers. |
| A-012 | `bin/run-stage.sh::main` re-executes `_clear_current_stage_slots` on every dispatch start regardless of prior dispatch's exit status (success, halt, retry-immediately). | **ASSUMED** — implementation must run the clear before any branching on prior state, immediately after dispatch_id allocation. Test pin asserts this. |
| A-013 | The atomic write pattern reuses `_cf_write_state` style (tmpfile + `mv -f`). | **Verified** at `bin/classify-failure.sh:31-37` — exactly the shape this design needs. |
| A-014 | `parse_pipeline_marker` (`bin/common.sh:192`) is the canonical marker reader; new `meta: dispatch` markers will be parsed by the same helper. | **Verified** — function exists; new bookkeeping kind requires a one-line addition to its kind-recognition table. |
| A-015 | `bin/verdict-handler.sh:84` (`find_fresh_verdict`) and `bin/verdict-handler.sh:330` (`resume_in_progress_transition`) are the two reader sites that need the dispatch_id-primary filter. | **Verified** by `grep -n` and direct read; both functions today reason from `createdAt` timestamps and `parse_pipeline_marker` outputs. |
...
Phase 1 (glue, no behavior change yet)
  ...
  ├─ bin/dispatch.sh: env passthrough at line 437
...
After Phase 3 ships and is stable for ~30 days, ENG-41's
labels-cross-check is demoted from primary to legacy-fallback (no
code change — the order of checks in `resume_in_progress_transition`
is reversed, dispatch_id-first, labels-second).

## 12. Out-of-scope future work

- **ENG-77 D-005 (typed findings channel).** Stays deferred under
  its existing un-deferral conditions. If un-deferred, layers on top
  of this design (typed-findings comments still get the
  auto-injection).
...
- **Cross-issue staleness.** The contract scopes per-issue. A future
  ...
  per-dispatch monotonic).
- **Transcript line tagging.** Open question §9.4. Track separately;
...
- **`bin/dispatch-test.sh`-style coverage for the auto-injection
  ...
  `dispatch.sh` (with stub claude) emits comments through
  `linear.sh` and the markers are present. Higher cost than the
  unit-level tests in §5.5; defer until empirical evidence the unit
  tests miss something.
...
on every dispatch start (by the new `allocate_dispatch_id`). Removed
only by `pipeline.sh decide --action continue` AND only when
`policy == "skip-until-human-acts"`. Atomic writes via `tmpfile + mv`
(the existing `_cf_write_state` at `bin/classify-failure.sh:31-37`
is reused).
...
```jsonc
{
  ...
  "issue":          "ENG-71",
  "stage":          "reviewing",
  "policy":         "retry-immediately",
  "reason":         "linear-post-failed",
  "exit_code":      24,
  "exit_subcode":   null,
  ...
  "branch":         "feat/eng-71-…",
  "evidence": {
    "pipeline_content_hash": "sha256:…",
    "branch_head_sha":       "4899db98…"
  },

  // ── NEW fields (dispatch_id glue) ─────────────────────────────────
  "current_dispatch_seq": 14,
  "current_dispatch_id":  "ENG-71-d014",
...
// Dispatch start:
{
  ...
  "trigger":      "transition|loopback|retry|inbox-pickup",
  "predecessor_dispatch_id": "ENG-71-d013",
  "branch":                  "feat/eng-71-…",
  "pipeline_content_hash":   "sha256:…"
}
```
...
// Dispatch end:
{
  ...
  "exit_code":     0,
  "policy":        null,
  "verdict_emitted": "fail|pass|halt|wait|null",
  "verdict_target":  "implementing",
  "duration_ms":    429000,
  "envelope":  {
    "stage_summary_present": true,
    "comments_stamped":      ["completion/reviewing/ENG-71"],
    "transcript_clean":      true
  }
}
```

The two-row shape lets retrospective + status.sh pair start with
end on `dispatch_id`; a missing end row indicates a mid-dispatch
crash. Schema-validated by a new helper in `bin/metrics.sh`.
...
Per D-003, the file's first line is added by the orchestrator
post-dispatch (NOT by the agent — agents must not manually write
dispatch-id markers): a `<!-- dispatch: <id> -->` line prepended
atomically before `post_completion_comment` reads. Forensic role
only — readers don't filter files by id (clear-at-start makes
...
Lifecycle: written by `_handle_wait` (`bin/run-stage.sh:489-545`)
on wait-shape exits; cleared by `_clear_current_stage_slots` at the
...
```jsonc
{
  ...
  "current_dispatch_id": "ENG-71-d014"   // NEW
}
```
...
|---|---|---|---|
| `completion/<stage>/<issue>` | orchestrator (`post_completion_comment`) | once per dispatch on success | wraps stage-summary file + auto-injected dispatch-id meta marker |
...
| `last-review-state/<issue>` | orchestrator | once per review dispatch | last reviewed PR HEAD SHA |
| `worktree-mutation/<issue>` | orchestrator (`bin/run-stage.sh:477` neighbourhood) | per worktree-mutation event | sweep result; partition_dirty_paths output |
| `halt/<stage>/<issue>` | classify-failure | per failure exit (skip-policies only) | verdict halt marker + policy + reason |
| `retry-pending/<stage>/<issue>` | classify-failure (post-ENG-78) | per retry-immediately exit | meta-shape "will retry" notice; not a verdict |
...
Bookkeeping markers stay in `<!-- meta: <kind> ... -->` shape. The
new auto-injection adds ONE new bookkeeping marker on every comment:
...
Each stage's hand-off has six slots. Loopback semantics noted only
on stages reachable by loopback (per the verified loopback table at
`bin/verdict-handler.sh:32-38`).

#### 13.2.1 Brainstorm (§1)

**Inputs (in read order):**
1. `docs/brainstorms/<existing-doc>.md` if present (frontmatter
   `linear: ENG-N`).
2. Linear issue body (full text).
3. `learned-rules/<project>/brainstorm.md`.
4. **Loopback** (planning/reviewing → brainstorming via supersede):
   most recent comment under sig `completion/<from-stage>/<issue>`
   carrying the supersede-trigger findings.

**Stage-summary file slots:** artifact link, TL;DR, single-line
status (clean), Notes (only on iteration), escalation line (if
iteration-exhausted).

**Linear writes during dispatch:**
- Append-only progress notes via `linear.sh add-comment` (free shape).
- Verdict via `bin/pipeline.sh event`:
  - `verdict pass --stage brainstorming`
  - `verdict halt --reason iteration-exhausted` (per ENG-65)
  - `verdict halt --reason agent-blocked`

**Orchestrator writes (post-dispatch):**
- `add-or-update-comment completion/brainstorming/<issue>`.
- Label transition (orchestrator lane): pass → adds `stage:planning`,
  removes `stage:brainstorming`, removes `pipeline:halted`.

**State writes:** `dispatch_history.jsonl`; `issue-state.json` (only
on failure); `stage-summary-brainstorming.md`.

#### 13.2.2 Plan (§2)

**Inputs:** brainstorm doc + `stage-summary-brainstorming.md` +
`completion/brainstorming/<issue>` + `learned-rules/<project>/plan.md`
+ loopback inputs.

**Verdicts:** pass / fail-to-brainstorming / halt.
**Orchestrator writes:** `completion/planning/<issue>`; transition.

#### 13.2.3 Implement (§3)

The most loopback-targeted stage.

**Inputs (loopback-aware):**
1. brainstorm + plan docs.
2. `stage-summary-implementing.md` IF present (only on retry-immediately
   re-dispatch within the same loopback cycle).
3. **Loopback inputs**: `stage-summary-reviewing.md`,
   `stage-summary-qa.md`, `stage-summary-building.md`;
   `completion/<source-stage>/<issue>`; verdict marker that triggered
   loopback.
4. Worktree state.
5. `learned-rules/<project>/implement.md`.

**Linear writes:**
- `add-or-update-comment tdd-evidence/implement/<issue>`.
- Verdict via `pipeline.sh event` (pass / fail-to-planning / halt).

**Loopback semantics:** when implement is re-entered, the previous
implement dispatch's stage-summary IS cleared at start. The new
dispatch reads the SOURCE stage's summary; it does NOT inherit its
own prior state.

#### 13.2.4 UI (§4)

**Inputs:** plan + `stage-summary-implementing.md` + worktree +
learned-rules.

**Verdicts:** pass / fail-to-implementing / halt.

**Orchestrator writes:** opens the PR (POST-dispatch — `gh pr create`);
PR body concatenates implement + ui stage-summary files.

**Not a loopback target** per the loopback table.

#### 13.2.5 Review (§5)

The stage where ENG-77's incident fired. Tightest loopback in the
pipeline.

**Inputs:** PR diff + brainstorm + plan + knowledge + learned-rules
+ `last-review-state/<issue>` + loopback summaries.

**Verdicts:**
- `verdict pass --stage reviewing` (path C)
- `verdict fail --target implementing` (path B)
- `verdict fail --target brainstorming` (path A — premise)
- `verdict halt --reason agent-blocked`

**Orchestrator writes:** `last-review-state/<issue>` (PR HEAD SHA);
label transitions.

**Loopback re-entry:** review's stage-summary file is cleared at the
start of EVERY review dispatch. The implement agent on subsequent
loopback reads the LATEST review's findings — guaranteed fresh.
ENG-71's 9-iteration cycle becomes structurally impossible.

#### 13.2.6 QA (§6)

**Inputs:** PR diff + brainstorm + plan + `stage-summary-reviewing.md`
+ `qa-patterns.md` (project) + learned-rules.

**Verdicts:** pass / fail-to-implementing / halt.

#### 13.2.7 Build (§7)

The only stage that emits wait-shape verdicts.

**Inputs:** PR + CI status + approval state + `stage-summary-qa.md`
+ learned-rules + `wait-building.json`.

**Verdicts:**
- `verdict pass --stage building`
- `verdict fail --target implementing` (conflict / CI red)
- `verdict halt --reason agent-blocked`
- `verdict wait --reason awaiting-approval` (P2)
- `verdict wait --reason awaiting-ci` (P5)
...
rewrite `wait-building.json` with fresh `current_dispatch_id`.

### 13.3 Linear label transitions per stage (orchestrator-only, lane-fenced)

| Stage transition | Labels added | Labels removed |
|---|---|---|
...
| reviewing → brainstorming (fail A) | `stage:brainstorming`, `pipeline:supersede` | `stage:reviewing`, `pipeline:halted` |
...
**Drift-prevention.** The table is enforced by
`bin/verdict-handler.sh::apply_transition` at the orchestrator side.
The new envelope validator asserts post-dispatch that the issue's
label state is consistent with the dispatch's verdict.
...
| `dispatch_history.jsonl` | does not exist | NEW — append-only, two rows per dispatch |
| `stage-summary-<stage>.md` | persisted across dispatches | cleared at start of OWN stage's dispatch; agent must rewrite; first line stamped by orchestrator post-dispatch |
| `wait-<stage>.json` | written on wait-exit; cleared on next dispatch start of same stage | + `current_dispatch_id` field |
| `completion/<stage>/<issue>` Linear comment | sig-deduped | Stamped with dispatch_id meta-marker (auto-injected) |
...
| `stage:*` labels | (today) mutated by orchestrator+agent | mutated only by orchestrator (post-ENG-41 lane fence) |
| `pipeline:halted` label | (today's bug) applied even on retry-immediately | applied only on skip-policies (post-ENG-78 fix) |

The only NEW persistent artifact is `dispatch_history.jsonl`.
Everything else is either an additive field on an existing JSON
file or a meta-marker injection on existing comments. No data
migration.

## 14. Persona review

Six personas per the brainstorm contract: design → security → scope
→ coherence → product → feasibility (gating). Verdicts and
load-bearing notes recorded here for the durable audit trail.

### 14.1 Design — PASS

The architecture splits cleanly: ONE glue (`dispatch_id`), FOUR
per-medium primitives (clear-on-start, auto-injection, lane-fence
[delegated to ENG-41], resolver registry), ONE detective backstop
(envelope validator). Each primitive matches its medium's persistence
semantics — no over-fit, no under-fit (§4.2). The extensibility
shape (§4.4) gives future contributors a forced choice from a small
menu rather than a blank slate.

Load-bearing tradeoff: the dispatch_id is per-issue monotonic, not
per-(issue, stage) (D-001). Forensic readability for "iter-N of
review" is reconstructed from `dispatch_history.jsonl` rather than
from the id itself; this is a deliberate choice to preserve total
ordering within an issue.

### 14.2 Security — PASS

- **Trust boundaries.** Auto-injection lives at the chokepoint
  (`bin/linear.sh:470/541`), not at the agent. Agents cannot forge
  a `dispatch_id` (D-003); the env var is allocated upstream and
  the lane-fenced linear.sh is the only authoring site for stamped
  comments. Operators retain manual-comment capability (env unset →
  no injection; reader fallback handles it).
- **Atomic state writes.** The allocator reuses `_cf_write_state`
  (`bin/classify-failure.sh:31-37`) — verified, race-free.
- **No new attack surface.** No new file readable by the agent
  (`dispatch_history.jsonl` is orchestrator-only). No new env var
  exposed to argv (auto-injection happens server-side at linear.sh).
- **Lane fence dependency is explicit.** Phase 3 cannot ship before
  ENG-41 lane fence (D-009). If shipped early, an agent that
  bypasses linear.sh via direct API can skirt auto-injection — the
  detective layer (§5.3 transcript scan) catches this, but the
  preventive primitive doesn't apply.
- **Secret handling.** No secret-pattern env vars introduced.

### 14.3 Scope — PASS

The Linear issue specifies:
- 5 invariants G-1 through G-5 — all addressed (§2).
- 4 per-medium primitives + 1 glue — exactly what §4 ships.
- 1 vocabulary expansion + 1 env var + 1 new persistent artifact —
  matches §13.4's diff table.
- 4-phase phasing — preserved (§11).
- Subsumes/un-defers/deferred relationships — explicit (D-006, D-007,
  D-008).

No scope creep. Phase 4 (resolver registry + envelope validator) is
the most ambitious but the issue explicitly names it as required.

### 14.4 Coherence — PASS

- **ENG-41 dependency.** Explicit (D-009): Phase 3 depends on lane
  fence shipping first. The labels-cross-check designed in ENG-41
  §4.2 demotes from primary to legacy-fallback once dispatch_id is
  in place.
- **ENG-77 relationship.** D-006 generalizes D-001 (§5 → §§1-7); D-007
  subsumes D-004; D-008 preserves D-005's deferral.
- **ENG-79 relationship.** ENG-79's single-resolver fix becomes the
  general case for `PROMPT_RESOLVERS` (Phase 4).
- **ENG-78 relationship.** Independent (open question §9.6); either
  ship order works.
- **Vocabulary fit.** New `meta: dispatch` kind and
  `dispatch-envelope-violation` halt reason fit the closed registry
  cleanly (A-009 verified).

### 14.5 Product — PASS

Solves the right problem: 6 incidents (~$50 + 2 manual interventions
on ENG-77 alone, plus operator hours across the others) collapse to
one class with one fix. Operator visibility preserved
(`bin/status.sh` reads `dispatch_history.jsonl`; `dispatch_id`
visible in halt comments and per-stage transcripts).

The detective layer (§5.3) is not a halt cudgel — it halts only on
EGREGIOUS bypass (D-004), so well-behaved agents never trip it.
Migration is amortized via the soft-fallback (D-005); no big-bang
deploy.

### 14.6 Feasibility — PASS (P0 = 0)

Codebase facts verified (Assumption Inventory §10 / IDs A-001
through A-015). One **correction** vs. the prior brainstorm draft
on the sibling branch `feat/cross-dispatch-staleness-handoff-contract-brainstorm`:
the rc=25 (`agent-contract-missing`) path lives at
`bin/run-stage.sh:1068-1080`, not 977-989 (the latter range is the
severe-scope-violation rc=21 path). This brainstorm cites the
verified line numbers throughout (§§4.2, 5.3, 8, A-002).

Code touch points all exist:
- `bin/common.sh:69-73` (issue_dir) ✓
- `bin/run-stage.sh:497-499` (wait-exit clear) ✓
- `bin/run-stage.sh:1068-1080` (rc=25 contract validator) ✓
- `bin/dispatch.sh:437` (env block) ✓
- `bin/linear.sh:470,541` (add_comment, add_or_update_comment) ✓
- `bin/render-prompt.sh:224` (branch-name resolver) ✓
- `bin/verdict-handler.sh:84,330,32-38` (find_fresh_verdict, resume_in_progress_transition, loopback table) ✓
- `bin/classify-failure.sh:31-37` (atomic-write pattern) ✓
- `bin/agent-prompts-content-test.sh:197-237` (ENG-77 §5 asserts to generalize) ✓
- `AGENT_PROMPTS.md:164-209,1027-1037` (Stage summary contract; ENG-77 §5 rule) ✓
- `bin/pipeline-events.json::meta_kinds, halt_reasons` ✓
- `bin/common.sh:111-138` (failure_outcome_for_exit; next free code is 29 — 27/28 in use) ✓
- `bin/common.sh:192` (parse_pipeline_marker) ✓

**Open assumption flagged but non-blocking:** A-007 (the agent's
`--allowed-tools` list excludes `mcp__plugin_linear`) is unverified
at brainstorm time — same caveat as ENG-41 §A-004. The transcript
scan in §5.3 is the backstop if the assumption is wrong.

**P0 findings: 0.** The remaining work is implementation; no
foundational gap blocks design progression to planning.

### 14.7 Persona summary
...
**6/6 PASS · gate P0: 0 · proceeding to planning.**
