...
```bash
# Label without file → respect the label; do not strip, do not include.
...
The log line names which label(s) are present and stops asserting the
*cause* (human vs pipeline race vs deleted-state-file) — only the
operator's investigation can disambiguate, and a recurring "human-
applied" line that is sometimes wrong reads worse than a recurring
neutral one. (Design persona P2, 2026-04-27.)
...
4. Append AC-9 (Bug B regression) to `bin/poll-slot-test.sh`:
   ...
   (b) `LINEAR_STUB_LOG` contains no exact line
   `remove-label ENG-XXXX pipeline:skip-until-human-acts` (use
   `grep -Fxq` for exact-line absence — a looser pattern over the stub
   log is brittle to future unrelated `remove-label` calls on the same
   issue ID).
5. AC-10 (required, not optional): same shape as AC-9 but with
   `pipeline:skip-until-code-changes` instead, asserting absence of the
   exact line `remove-label ENG-XXXX pipeline:skip-until-code-changes`
   via `grep -Fxq`. D-2's rationale rests on "uniform treatment is
   correct, not a regression"; the test that proves uniformity must
   therefore be a requirement, not optional.
...
Document-review run 2026-04-27 (re-confirmation pass). Six personas,
executed in the prescribed order (design → security → scope →
coherence → product → feasibility). Feasibility runs last because it
is the gating persona.

| # | Persona | Verdict | Findings |
|---|---|---|---|
| 1 | Design (operator/decision-point + edge cases + observability + AI-slop) | PASS · P0/P1 = 0 | UI-only axes N/A (backend bash). Decision-point coverage 9/10 (§4 traces every branch pre/post). Edge-case enumeration 9/10 (§6 covers seven cases including races). AI-slop 10/10. P2 (folded into §2 D-2): orphan-branch log line should name which label(s) are present (`code=…` and `human=…`) instead of asserting "(human-applied)" — the recurring tail-log message is the only operator-facing signal for a stage-labeled issue idling for hours, and the "(human-applied)" parenthetical can be wrong (mid-tick race, deleted state file). P2 (folded into §9 steps 4–5): AC-9/AC-10 negative assertions specified as exact-line absence via `grep -Fxq` so a future unrelated `remove-label` on the same issue ID cannot silently relax the regression guard. |
| 2 | Security | PASS · P0/P1/P2 = 0 | Fix repairs the kill-switch contract correctly. Net Linear-write surface shrinks (1–2 fewer `remove-label` calls per orphan-labeled issue per tick). No new attack surface. Single-tick race window is the same property `pipeline:paused` already has — accepted, not introduced. AuthN/AuthZ/XSS/CSRF/SQLi/PII/secrets all explicitly N/A for harness bash. Plan-level threats walked: (a) wrong test seeding masking the regression — mitigated by §9 step 4's explicit "no state file" instruction; (b) label-name typo desynchronizing filter from contract — implementer should copy-paste from `bin/setup-labels.sh:37-38`; (c) classify-failure race writing state file after human applies skip — §6 case 6 walks the next-tick path correctly. |
| 3 | Scope | PASS · P0/P1/P2 = 0 | Two file edits, ~10 lines net diff, two/three test cases. 1:1 map to Linear IN scope. All four OUT items called out as non-goals. Four rejected alternatives in §2 (D-1 alt; two D-2 alts; D-3 alt) turn down the over-engineered options. AC-10 is required (not optional), preserving the uniformity-of-treatment claim. |
| 4 | Coherence | PASS · P0/P1/P2 = 0 | No contradictions. Terminology used uniformly ("orphan label" / "label without state file" used interchangeably with the term defined inline). Uniformity-of-treatment claim consistent across §1, §2 D-2, §4 data flow, §5 error handling, §6 edge cases, §8.3 assumptions, §9 ACs. Forward references all grounded in §8.4. |
| 5 | Product | PASS · P0/P1 = 0 | Right problem (operator kill-switch contract is contradicted by code; ENG-23 prep surfaced it). Direct, single-hop fix at the two contradicting sites — no proxy work, no rollout machinery. Goal-requirement alignment 1:1 (Bug A → D-1, Bug B → D-2, regression → D-3). P2: do-nothing pain qualitatively asserted, not quantified — acceptable for a two-line fix where the failure class is binary (a kill switch silently bypassable has zero acceptable failure rate). |
| 6 | Feasibility (gating) | PASS · P0/P1 = 0 | Every named code artifact re-verified against the current worktree at HEAD: `bin/poll.sh:45-116` (`_poll_evaluate_skip`), `bin/poll.sh:64-69` (orphan-label branch — matches issue snippet exactly), `bin/poll.sh:186-232` (`_poll_classify_labels`; non-zero return → `slot:vacate, advanceable:false` at L190-193), `bin/poll.sh:432-439` (Pass 5 inbox jq filter — issue cited 316-325, actual is 432-439, structure identical; documented in §8.3 #3), `bin/poll.sh:452-454` (sentinel), `bin/common.sh:61-65` (`issue_dir`), `bin/common.sh:71-88, 148` (`compute_pipeline_content_hash` definition + export), `bin/setup-labels.sh:37-38` (label contracts), `bin/classify-failure.sh:101-107` (pipeline-side label apply), `bin/poll-slot-test.sh:62-87, 159-206, 366-446` (test scaffolding + AC-6/AC-7). P2 (cosmetic): two off-by-one line citations in the doc (§3 "classify-failure.sh:101-107" — actual case block runs 100-108; §8.3 #8 "367-446" — AC-6 starts at 366). Cited content matches exactly; offsets are within ±1 line. Not blocking. |

Result: **6/6 PASS · gate P0 = 0 · gate P1 = 0**. Proceeding to planning.
