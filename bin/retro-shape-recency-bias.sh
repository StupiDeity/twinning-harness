#!/usr/bin/env bash
# Shape: recency-bias. Pre-computes the recency-bias section of the
# weekly retrospective. Output: a markdown artifact at $artifact_path
...
#   iterates <learned_rules_dir>/*.md to classify active rules by domain.
...
_render_prompt() {
  local rendered="$1"
  local template="$HARNESS_ROOT/bin/retro-prompts/recency-bias.md"
...
main() {
  ...
  local rendered
  rendered="$(mktemp -t retro-shape-recency-bias-XXXXXX.md)"

  _render_prompt "$rendered"
  _validate_no_unresolved_tokens "$rendered"

  if [[ "${PIPELINE_DRY_RUN-}" == "1" ]]; then
    log "[DRY_RUN] would dispatch.sh retrospective with prompt=$rendered artifact=$_ARTIFACT_PATH (shape=recency-bias)"
    printf '%s\n' '[DRY_RUN placeholder] recency-bias' > "$_ARTIFACT_PATH"
    rm -f "$rendered"
    return 0
  fi

  local log_file
  log_file="$PROJECT_STATE_DIR/logs/retro-shape-recency-bias-$(date -u +%Y%m%dT%H%M%SZ).log"
