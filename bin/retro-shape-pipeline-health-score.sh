#!/usr/bin/env bash
# Shape: pipeline-health-score. Pre-computes the pipeline-health-score
...
_render_prompt() {
  local rendered="$1"
  local template="$HARNESS_ROOT/bin/retro-prompts/pipeline-health-score.md"
...
main() {
  ...
  local rendered
  rendered="$(mktemp -t retro-shape-pipeline-health-score-XXXXXX.md)"

  _render_prompt "$rendered"
  _validate_no_unresolved_tokens "$rendered"

  if [[ "${PIPELINE_DRY_RUN-}" == "1" ]]; then
    log "[DRY_RUN] would dispatch.sh retrospective with prompt=$rendered artifact=$_ARTIFACT_PATH (shape=pipeline-health-score)"
    printf '%s\n' '[DRY_RUN placeholder] pipeline-health-score' > "$_ARTIFACT_PATH"
    rm -f "$rendered"
    return 0
  fi

  local log_file
  log_file="$PROJECT_STATE_DIR/logs/retro-shape-pipeline-health-score-$(date -u +%Y%m%dT%H%M%SZ).log"
