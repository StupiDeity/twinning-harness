#!/usr/bin/env bash
# Tests for bin/retro-shape-pipeline-health-score.sh.
...
if [[ "$write_artifact" == "yes" ]]; then
  ...
  if [[ -n "\$artifact_path" ]]; then
    printf '## Pipeline health\n\ninsufficient-sample (N=0, need >=5)\n' > "\$artifact_path"
  fi
fi
exit $rc
STUB
...
source "$HARNESS_DIR/retro-shape-pipeline-health-score.sh"
SCRIPT_DIR="$STUB_DIR"
...
printf '\n'
if (( FAIL == 0 )); then
  printf 'OK: retro-shape-pipeline-health-score tests (%d passed)\n' "$PASS"
  exit 0
else
