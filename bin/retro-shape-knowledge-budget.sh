#!/usr/bin/env bash
# Shape: knowledge-budget. Pre-computes the knowledge-budget section of
...
_render_prompt() {
  local rendered="$1"
  local template="$HARNESS_ROOT/bin/retro-prompts/knowledge-budget.md"
...
main() {
  ...
  local rendered
  rendered="$(mktemp -t retro-shape-knowledge-budget-XXXXXX.md)"

  _render_prompt "$rendered"
  _validate_no_unresolved_tokens "$rendered"

  if [[ "${PIPELINE_DRY_RUN-}" == "1" ]]; then
    log "[DRY_RUN] would dispatch.sh retrospective with prompt=$rendered artifact=$_ARTIFACT_PATH (shape=knowledge-budget)"
    printf '%s\n' '[DRY_RUN placeholder] knowledge-budget' > "$_ARTIFACT_PATH"
    rm -f "$rendered"
    return 0
  fi

  local log_file
  log_file="$PROJECT_STATE_DIR/logs/retro-shape-knowledge-budget-$(date -u +%Y%m%dT%H%M%SZ).log"
