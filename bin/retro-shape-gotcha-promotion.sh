#!/usr/bin/env bash
# Shape: gotcha-promotion. Pre-computes the gotcha-promotion section of
...
_render_prompt() {
  local rendered="$1"
  local template="$HARNESS_ROOT/bin/retro-prompts/gotcha-promotion.md"
...
main() {
  ...
  local rendered
  rendered="$(mktemp -t retro-shape-gotcha-promotion-XXXXXX.md)"

  _render_prompt "$rendered"
  _validate_no_unresolved_tokens "$rendered"

  if [[ "${PIPELINE_DRY_RUN-}" == "1" ]]; then
    log "[DRY_RUN] would dispatch.sh retrospective with prompt=$rendered artifact=$_ARTIFACT_PATH (shape=gotcha-promotion)"
    printf '%s\n' '[DRY_RUN placeholder] gotcha-promotion' > "$_ARTIFACT_PATH"
    rm -f "$rendered"
    return 0
  fi

  local log_file
  log_file="$PROJECT_STATE_DIR/logs/retro-shape-gotcha-promotion-$(date -u +%Y%m%dT%H%M%SZ).log"
