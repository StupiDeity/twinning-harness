#!/usr/bin/env bash
# Tests for bin/retro-shape-survivorship-bias.sh.
...
source "$HARNESS_DIR/retro-shape-survivorship-bias.sh"
SCRIPT_DIR="$STUB_DIR"
...
printf '\n'
if (( FAIL == 0 )); then
  printf 'OK: retro-shape-survivorship-bias tests (%d passed)\n' "$PASS"
  exit 0
else
