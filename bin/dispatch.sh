...
allowed_tools_for() {
  ...
  case "$1" in
    ...
    # git add/commit are LOAD-BEARING: AGENT_PROMPTS.md §2 step 4 directs the
    # plan agent to commit the plan .md + sibling .json, and run-stage.sh's
    # _validate_plan_contract (ENG-179) halts exit=35 (plan-contract-missing)
    # unless that plan is in branch HEAD. git status is for the agent's own
    # staging verification. Omitting these was masked while older claude CLIs
    # ran non-allowlisted Bash anyway; tightened headless enforcement turned
    # the gap into a hard halt on every fresh plan dispatch (ENG-194).
    planning)       base='Read,Write,Edit,Grep,Glob,TaskCreate,Bash(git log:*),Bash(git diff:*),Bash(git status:*),Bash(git add:*),Bash(git commit:*),Bash(bash .pipeline/bin/linear.sh:*),Bash(bash bin/linear.sh:*),Bash(bash .pipeline/bin/pipeline.sh:*),Bash(bash bin/pipeline.sh:*),Bash(bash .pipeline/bin/plan-schema.sh:*),Bash(bash bin/plan-schema.sh:*)' ;;
    ...
    qa)             base='Read,Write,Edit,Grep,Glob,TaskCreate,Agent,Bash(git:*),Bash(jq:*),Bash(awk:*),Bash(gh pr view:*),Bash(gh pr diff:*),Bash(gh pr comment:*),Bash(gh issue create:*),Bash(gh issue list:*),Bash(bash .pipeline/bin/linear.sh:*),Bash(bash bin/linear.sh:*),Bash(bash .pipeline/bin/pipeline.sh:*),Bash(bash bin/pipeline.sh:*),Bash(bash .pipeline/bin/guards.sh:*),Bash(bash bin/guards.sh:*),Bash(bash .pipeline/bin/verify-qa.sh:*),Bash(bash bin/verify-qa.sh:*)' ;;
