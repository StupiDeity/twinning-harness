#!/usr/bin/env bash
# Tests for bin/retro-shape-knowledge-budget.sh.
...
source "$HARNESS_DIR/retro-shape-knowledge-budget.sh"
SCRIPT_DIR="$STUB_DIR"
...
printf '\n'
if (( FAIL == 0 )); then
  printf 'OK: retro-shape-knowledge-budget tests (%d passed)\n' "$PASS"
  exit 0
else
