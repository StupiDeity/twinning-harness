#!/usr/bin/env bash
# Tests for bin/retro-shape-prompt-workflow-amendment.sh.
...
source "$HARNESS_DIR/retro-shape-prompt-workflow-amendment.sh"
SCRIPT_DIR="$STUB_DIR"
...
printf '\n'
if (( FAIL == 0 )); then
  printf 'OK: retro-shape-prompt-workflow-amendment tests (%d passed)\n' "$PASS"
  exit 0
else
