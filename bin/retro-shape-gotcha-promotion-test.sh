#!/usr/bin/env bash
# Tests for bin/retro-shape-gotcha-promotion.sh.
...
source "$HARNESS_DIR/retro-shape-gotcha-promotion.sh"
SCRIPT_DIR="$STUB_DIR"
...
printf '\n'
if (( FAIL == 0 )); then
  printf 'OK: retro-shape-gotcha-promotion tests (%d passed)\n' "$PASS"
  exit 0
else
