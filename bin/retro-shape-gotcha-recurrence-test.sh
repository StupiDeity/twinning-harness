#!/usr/bin/env bash
# Tests for bin/retro-shape-gotcha-recurrence.sh.
...
source "$HARNESS_DIR/retro-shape-gotcha-recurrence.sh"
SCRIPT_DIR="$STUB_DIR"
...
# fixture-3: token resolution — non-DRY_RUN
{
...
# fixture-4: DRY_RUN does not invoke dispatch
{
  ...
  if (( rc == 0 )) && [[ "$dispatch_called" == "no" ]] && [[ "$placeholder_exists" == "yes" ]]; then
    _pass "$name"
  else
...
# fixture-5: artifact missing → die
{
...
# fixture-6: dispatch failure (rc=29) → die
{
...
printf '\n'
if (( FAIL == 0 )); then
  printf 'OK: retro-shape-gotcha-recurrence tests (%d passed)\n' "$PASS"
  exit 0
else
