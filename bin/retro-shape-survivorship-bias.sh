#!/usr/bin/env bash
# Shape: survivorship-bias. Pre-computes the survivorship-bias section of
...
_render_prompt() {
  local rendered="$1"
  local template="$HARNESS_ROOT/bin/retro-prompts/survivorship-bias.md"
...
main() {
  ...
  local rendered
  rendered="$(mktemp -t retro-shape-survivorship-bias-XXXXXX.md)"

  _render_prompt "$rendered"
  _validate_no_unresolved_tokens "$rendered"

  if [[ "${PIPELINE_DRY_RUN-}" == "1" ]]; then
    log "[DRY_RUN] would dispatch.sh retrospective with prompt=$rendered artifact=$_ARTIFACT_PATH (shape=survivorship-bias)"
    printf '%s\n' '[DRY_RUN placeholder] survivorship-bias' > "$_ARTIFACT_PATH"
    rm -f "$rendered"
    return 0
  fi

  local log_file
  log_file="$PROJECT_STATE_DIR/logs/retro-shape-survivorship-bias-$(date -u +%Y%m%dT%H%M%SZ).log"
