#!/usr/bin/env bash
# Shape: tool-denial-trends. Pre-computes claude_version x stage tool-denial
# gradient from events.jsonl::sandbox_denial rows for the weekly retrospective.
# Output: a markdown artifact at $artifact_path consumed by the parent
# retrospective's §9 prompt via {tool_denial_trends_path} interpolation.
...
_render_prompt() {
  local rendered="$1"
  local template="$HARNESS_ROOT/bin/retro-prompts/tool-denial-trends.md"
...
main() {
  ...
  local rendered
  rendered="$(mktemp -t retro-shape-tool-denial-trends-XXXXXX.md)"
  ...
  if [[ "${PIPELINE_DRY_RUN-}" == "1" ]]; then
    ...
    printf '%s\n' '[DRY_RUN placeholder] tool-denial-trends' > "$_ARTIFACT_PATH"
    rm -f "$rendered"
    return 0
  fi

  local log_file
  log_file="$PROJECT_STATE_DIR/logs/retro-shape-tool-denial-trends-$(date -u +%Y%m%dT%H%M%SZ).log"
