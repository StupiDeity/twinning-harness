#!/usr/bin/env bash
# Shape: prompt-workflow-amendment. Pre-computes the prompt-workflow-amendment
# section of the weekly retrospective. Output: a markdown artifact at
# $artifact_path concatenated into the coordinator's PR body.
...
_render_prompt() {
  local rendered="$1"
  local template="$HARNESS_ROOT/bin/retro-prompts/prompt-workflow-amendment.md"
...
main() {
  ...
  local rendered
  rendered="$(mktemp -t retro-shape-prompt-workflow-amendment-XXXXXX.md)"

  _render_prompt "$rendered"
  _validate_no_unresolved_tokens "$rendered"

  if [[ "${PIPELINE_DRY_RUN-}" == "1" ]]; then
    log "[DRY_RUN] would dispatch.sh retrospective with prompt=$rendered artifact=$_ARTIFACT_PATH (shape=prompt-workflow-amendment)"
    printf '%s\n' '[DRY_RUN placeholder] prompt-workflow-amendment' > "$_ARTIFACT_PATH"
    rm -f "$rendered"
    return 0
  fi

  local log_file
  log_file="$PROJECT_STATE_DIR/logs/retro-shape-prompt-workflow-amendment-$(date -u +%Y%m%dT%H%M%SZ).log"
