#!/usr/bin/env bash
# Shape: confirmation-bias-audit. Pre-computes the confirmation-bias-audit
...
_render_prompt() {
  local rendered="$1"
  local template="$HARNESS_ROOT/bin/retro-prompts/confirmation-bias-audit.md"
...
main() {
  ...
  local rendered
  rendered="$(mktemp -t retro-shape-confirmation-bias-audit-XXXXXX.md)"

  _render_prompt "$rendered"
  _validate_no_unresolved_tokens "$rendered"

  if [[ "${PIPELINE_DRY_RUN-}" == "1" ]]; then
    log "[DRY_RUN] would dispatch.sh retrospective with prompt=$rendered artifact=$_ARTIFACT_PATH (shape=confirmation-bias-audit)"
    printf '%s\n' '[DRY_RUN placeholder] confirmation-bias-audit' > "$_ARTIFACT_PATH"
    rm -f "$rendered"
    return 0
  fi

  local log_file
  log_file="$PROJECT_STATE_DIR/logs/retro-shape-confirmation-bias-audit-$(date -u +%Y%m%dT%H%M%SZ).log"
