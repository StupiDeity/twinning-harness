#!/usr/bin/env bash
# Tests for bin/retro-shape-confirmation-bias-audit.sh.
...
if [[ "$write_artifact" == "yes" ]]; then
  ...
  if [[ -n "\$artifact_path" ]]; then
    printf '## Bias findings\n\n- Confirmation-bias cycles: none\n- Cross-agent contradictions: none\n- Rules renewed >=3x: none\n' > "\$artifact_path"
  fi
fi
exit $rc
STUB
...
source "$HARNESS_DIR/retro-shape-confirmation-bias-audit.sh"
SCRIPT_DIR="$STUB_DIR"
...
printf '\n'
if (( FAIL == 0 )); then
  printf 'OK: retro-shape-confirmation-bias-audit tests (%d passed)\n' "$PASS"
  exit 0
else
