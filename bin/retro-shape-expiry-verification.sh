#!/usr/bin/env bash
# Shape: expiry-verification. Pre-computes the expiry-verification section
# of the weekly retrospective. Output: a markdown artifact at $artifact_path
# concatenated into the coordinator's PR body.
#
# Per-shape token beyond the five baseline:
#   {learned_rules_dir} — per-project learned-rules directory; this shape
#   reads <learned_rules_dir>/*.md for expired learned-rule entries.
...
_render_prompt() {
  local rendered="$1"
  local template="$HARNESS_ROOT/bin/retro-prompts/expiry-verification.md"
  [[ -f "$template" ]] || die "shape: prompt template not found: $template"
  local learned_rules_dir="$HARNESS_ROOT/learned-rules/${PROJECT_SLUG}"
  sed \
...
_validate_no_unresolved_tokens() {
  local rendered="$1"
  local unresolved=""
  for tok in events_jsonl_path period_start_iso period_end_iso artifact_path previous_period_path learned_rules_dir; do
  ...
  extra="$(grep -oE '\{[a-z][a-z_]*[a-z]\}' "$rendered" 2>/dev/null \
    | grep -vE '^\{(artifact_path|events_jsonl_path|period_start_iso|period_end_iso|previous_period_path|learned_rules_dir)\}$' \
    | head -1 || true)"
...
main() {
  ...
  local rendered
  rendered="$(mktemp -t retro-shape-expiry-verification-XXXXXX.md)"

  _render_prompt "$rendered"
  _validate_no_unresolved_tokens "$rendered"

  if [[ "${PIPELINE_DRY_RUN-}" == "1" ]]; then
    log "[DRY_RUN] would dispatch.sh retrospective with prompt=$rendered artifact=$_ARTIFACT_PATH (shape=expiry-verification)"
    printf '%s\n' '[DRY_RUN placeholder] expiry-verification' > "$_ARTIFACT_PATH"
    rm -f "$rendered"
    return 0
  fi

  local log_file
  log_file="$PROJECT_STATE_DIR/logs/retro-shape-expiry-verification-$(date -u +%Y%m%dT%H%M%SZ).log"
