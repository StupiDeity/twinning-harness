#!/usr/bin/env bash
# Tests for bin/retro-shape-convention-drift.sh.
...
source "$HARNESS_DIR/retro-shape-convention-drift.sh"
SCRIPT_DIR="$STUB_DIR"
...
printf '\n'
if (( FAIL == 0 )); then
  printf 'OK: retro-shape-convention-drift tests (%d passed)\n' "$PASS"
  exit 0
else
