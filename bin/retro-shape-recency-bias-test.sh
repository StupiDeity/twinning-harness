#!/usr/bin/env bash
# Tests for bin/retro-shape-recency-bias.sh.
...
source "$HARNESS_DIR/retro-shape-recency-bias.sh"
SCRIPT_DIR="$STUB_DIR"
...
printf '\n'
if (( FAIL == 0 )); then
  printf 'OK: retro-shape-recency-bias tests (%d passed)\n' "$PASS"
  exit 0
else
