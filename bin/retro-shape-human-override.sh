#!/usr/bin/env bash
# Shape: human-override. Pre-computes the human-override section of
...
#   {project_profile_path} — read its `## File layout` to know which
#   code-bearing directories to inspect for human overrides.
...
_render_prompt() {
  local rendered="$1"
  local template="$HARNESS_ROOT/bin/retro-prompts/human-override.md"
  [[ -f "$template" ]] || die "shape: prompt template not found: $template"
  local project_profile_path="$HARNESS_ROOT/learned-rules/${PROJECT_SLUG}/project-profile.md"
  sed \
    ...
    -e "s|{project_profile_path}|${project_profile_path}|g" \
    "$template" > "$rendered"
}
...
_validate_no_unresolved_tokens() {
  ...
  extra="$(grep -oE '\{[a-z][a-z_]*[a-z]\}' "$rendered" 2>/dev/null \
    | grep -vE '^\{(artifact_path|events_jsonl_path|period_start_iso|period_end_iso|previous_period_path|project_profile_path)\}$' \
    | head -1 || true)"
...
main() {
  ...
  local rendered
  rendered="$(mktemp -t retro-shape-human-override-XXXXXX.md)"

  _render_prompt "$rendered"
  _validate_no_unresolved_tokens "$rendered"

  if [[ "${PIPELINE_DRY_RUN-}" == "1" ]]; then
    log "[DRY_RUN] would dispatch.sh retrospective with prompt=$rendered artifact=$_ARTIFACT_PATH (shape=human-override)"
    printf '%s\n' '[DRY_RUN placeholder] human-override' > "$_ARTIFACT_PATH"
    rm -f "$rendered"
    return 0
  fi

  local log_file
  log_file="$PROJECT_STATE_DIR/logs/retro-shape-human-override-$(date -u +%Y%m%dT%H%M%SZ).log"
