#!/usr/bin/env bash
# Tests for bin/retro-shape-expiry-verification.sh.
...
if [[ "$write_artifact" == "yes" ]]; then
  ...
  if [[ -n "\$artifact_path" ]]; then
    printf '## Expiry decisions (every expired entry must appear here)\n\nnone\n' > "\$artifact_path"
  fi
fi
exit $rc
STUB
...
source "$HARNESS_DIR/retro-shape-expiry-verification.sh"
SCRIPT_DIR="$STUB_DIR"
...
# fixture-7: per-shape extra token (learned_rules_dir) renders
{
  name="fixture-7-extra-token-learned-rules-dir-rendered"
  ...
  main --artifact-path "$artifact_path" --period-start-iso 2026-05-08T00:00:00Z --period-end-iso 2026-05-15T00:00:00Z 2>/dev/null || rc=$?
  export PIPELINE_DRY_RUN=1
  if [[ -f "$RENDERED_PROMPT_COPY" ]] && grep -qF "learned-rules/${PROJECT_SLUG}" "$RENDERED_PROMPT_COPY"; then
    _pass "$name"
  else
    _fail "$name (learned_rules_dir not substituted; rc=$rc)"
  fi
  export SHAPE_TEST_ARTIFACT_PATH=""
}
...
printf '\n'
if (( FAIL == 0 )); then
  printf 'OK: retro-shape-expiry-verification tests (%d passed)\n' "$PASS"
  exit 0
else
