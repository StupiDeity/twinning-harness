...
_render_prompt() {
  local rendered="$1"
  local template="$HARNESS_ROOT/bin/retro-prompts/stage-failure-summary.md"
...
_validate_no_unresolved_tokens() {
  local rendered="$1"
  # The prompt template contains illustrative {field} patterns inside jq
  # filter examples; only our five declared tokens should have been resolved.
  # Check that none of the five declared tokens remain.
  local unresolved=""
  ...
  # Also check for any remaining snake_case tokens that look like missed substitutions.
  local extra
...
main() {
  ...
  local rendered
  rendered="$(mktemp -t retro-shape-stage-failure-summary-XXXXXX.md)"
  ...
  if [[ "${PIPELINE_DRY_RUN-}" == "1" ]]; then
    ...
    printf '%s\n' '[DRY_RUN placeholder] stage-failure-summary' > "$_ARTIFACT_PATH"
    rm -f "$rendered"
    return 0
  fi

  local log_file
  log_file="$PROJECT_STATE_DIR/logs/retro-shape-stage-failure-summary-$(date -u +%Y%m%dT%H%M%SZ).log"
