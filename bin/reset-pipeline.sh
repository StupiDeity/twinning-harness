...
#
# Clears:
#   - $PROJECT_STATE_DIR/.consecutive-failures     (circuit-breaker counter)
#   - orchestrator.paused in $STATE_FILE           (flipped back to false via set_orchestrator_paused)
#   - $PROJECT_STATE_DIR/<ID>/issue-state.json     (per-issue classify_failure record)
...
main() {
  ...
  if [[ -f "$PROJECT_STATE_DIR/.consecutive-failures" ]]; then
    rm -f "$PROJECT_STATE_DIR/.consecutive-failures"
    log "cleared $PROJECT_STATE_DIR/.consecutive-failures"
  fi
